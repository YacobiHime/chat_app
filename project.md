...
| 検索 | DuckDuckGo Instant Answer API / Bluesky API |
...
│   └── web-search.ts             # DuckDuckGo 検索ユーティリティ
│
├── prisma/
...
検出後、DuckDuckGo または Bluesky API で検索し、結果をシステムメッセージとして追加したうえで再度 Ollama にリクエストを送る。**ツール呼び出しは 1 回のみ**（二重呼び出しは無視）。
...
- `app/api/chat/route.ts` 内の `toolCallMatch` は定義されておらず、**参照エラー（バグ）** がある（`blueskySearchMatch` 検出後の `buffer` 更新処理 `buffer.substring(toolCallMatch.index! + ...)` の箇所）。
- `data/plots.json` は旧バージョンの遺物。現在のデータは SQLite DB（Prisma）で管理されており、このファイルは使用されていない。
- `lib/web-search.ts` は独立したユーティリティだが、現在のチャット API では `route.ts` 内にインライン実装した検索ロジックが使われており、このファイルは直接呼ばれていない。
